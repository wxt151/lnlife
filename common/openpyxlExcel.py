...
class READEXCEL:
    ...
    def position_sheet_row_value(self, min_row=1, max_row=None, max_col=None):
        ...
        for row in content:
            row_data = []
            for single in row:
                row_data.append(str(single.value))
...
class OpenExcelPandas(READEXCEL, PANDASDATA):
    ...
    def readCaseExcel(self,title='函数'):
        ...
        self._data = self.position_sheet_row_value()
        # print(self._data)
        # 获取指定标题的内容
    ...
    def internal_read_excel(self, title="函数"):
        ...
        column = list(self._data[title])  # 设置序列号的名字
        # return self.conversionPandas(columnLabel)
        return self.conversion_column(self._data,column)
    ...
    def conversionPandas(self, title='函数'):
        ...
        self._data = self.dataFrame()  # 设置标题名
        self._title = list(self._data.iloc[0])
        self._data = self._data[1:len(self._data)]
        print(self._data)
        self._data = self.dataFrame(columns=self._title)
        print(self._data)
...
if __name__ == '__main__':
    file_path = r"E:\wxt\测试相关\测试用例\用户数据"
    file_name_xlsx = r"\300.xlsx"
    read = OpenExcelPandas(file_path + file_name_xlsx,sheet = "300")
